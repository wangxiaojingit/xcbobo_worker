...
var myapp=new Vue({
    ...
    created:function(){
    	...
    	if(parameter().isShare==0){
        	...
        	//辉煌新增逻辑,在app内,如果在我的商品页面,分享成功之后,我们就跳转到这个页面
        	if(parameter().leave_money){
        		that.leave_money=parameter().leave_money||0;
        		that.cut1_money=parameter().cut1_money||0;
    	...
    	if(parameter().isShare==1){
           ...
           // 进来的时候如果是web 就去配置微信   	
        	that.config();
        	//that.getOpenId();//获取union_id//如果是分享出去的页面,必须拿到这个union_id
        }
    	  ...
    	  weixinCallback:function(appId,timestamp,nonceStr,signature){
    		  var that=this;
    	       ...
    	       wx.ready(function(){
                   ...
                   //分享链接
                   var e=location.href.split("message2");
                   var shareurl=e[0]+"message"+e[1];
                   var link=base+"/xcbb_web/auth2.html?url="+shareurl.split('#')[0];
    	           //如果支持分享给朋友圈的时候开始!
